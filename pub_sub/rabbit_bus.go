...
func NewRabbitBus(cfg *RabbitMQConfig) (interfaces.Bus, error) {
...
func (rb *rabbitBus) Subscribe(topic string, cb func(data interface{}), options *interfaces.QueueOptions) (func(), error) {
	id := uuid.New()
	...
	if _, ok := rb.queuesMap.Load(topic); !ok {
		...
		if options == nil {
			options = &interfaces.QueueOptions{ // default
				Durable:    false,
				AutoDelete: true,
				Exclusive:  false,
				NoWait:     false,
