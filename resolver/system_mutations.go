...
func (s *GraphQLServer) UpsertFieldToModelResolverFn(p graphql.ResolveParams) (interface{}, error) {
	...
	fieldInfo, err := s.searchFields(&modelType.Fields, parentField, identifier, false)
	if err != nil {
		return nil, err
	}
...
func (s *GraphQLServer) rearrangeField(modelType *models.ModelType, draggedFieldName, moveType string, newPosition int, parentId string) error {
	...
	if parentId == "" {
		...
		sourceParent, _ := s.searchFields(&modelType.Fields, "", parentId, false)
		if sourceParent == nil {
	...
	case "parent_to_child":
		...
		targetParent, _ := s.searchFields(&modelType.Fields, "", parentId, false)
		if targetParent == nil {
...
func (s *GraphQLServer) ModelFieldOperationResolverFn(p graphql.ResolveParams) (interface{}, error) {
	...
	case enums.FieldOperation_Delete:
		if isRelation {
			...
			// delete the field
			deletedField, err := s.searchFields(&modelType.Fields, parentField, fieldName, true)
			if err != nil {
				return nil, err
			}
...
func (s *GraphQLServer) searchFields(fields *[]*models.FieldInfo, parentIdentifier, identifier string, delete bool) (*models.FieldInfo, error) {
	if identifier == "" {
		return nil, errors.New("identifier is required")
	}
	if len(*fields) == 0 {
		return nil, errors.New("fields array is required")
	}
		...
		for _, sub := range parentField.SubFieldInfo {
			if sub.Identifier == identifier {
				if delete {
					...
					index, deleted, found := deleteFieldByIdentifier(parentField.SubFieldInfo, identifier)
					if found {
			...
			if len(sub.SubFieldInfo) > 0 {
				if found, err := s.searchFields(&sub.SubFieldInfo, "", identifier, delete); err == nil && found != nil {
					return found, nil
				}
			}
		}
	...
	for _, f := range *fields {
		if f.Identifier == identifier {
			if delete {
				index, deleted, found := deleteFieldByIdentifier(*fields, identifier)
				if found {
					// Remove the element at the found index
...
func (s *GraphQLServer) deleteRelationField(ctx context.Context, project *models.Project, modelType *models.ModelType, identifier string) (*models.ConnectionType, *models.ConnectionType, error) {
	...
	if fromConnectionType.Relation == "has_one" {
		_, err := s.searchFields(&modelType.Fields, "", fmt.Sprintf(`system_%s_id`, fromConnectionType.Model), true)
		if err != nil {
			return nil, nil, err
		}
	}
	...
	if toConnectionType.Relation == "has_one" {
		_, err := s.searchFields(&modelType.Fields, "", fmt.Sprintf(`system_%s_id`, toConnectionType.Model), true)
		if err != nil {
			return nil, nil, err
		}
	}
