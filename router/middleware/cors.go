...
func CORSMiddleware(cfg *models.Config) echo.MiddlewareFunc {
	return func(next echo.HandlerFunc) echo.HandlerFunc {
		return func(c echo.Context) (err error) {
			...
			c.Response().Header().Set("Access-Control-Allow-Headers", "Origin, Content-Type, Content-Length, X-USE-Cookies, X-Apito-Key, X-Apito-Tenant-ID, X-Fn-Hash, X-Requested-With, Accept-Encoding, X-CSRF-Token, Authorization")
