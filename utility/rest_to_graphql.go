...
func getFieldsInfo(fields map[string]interface{}, _fields []*protobuff.FieldInfo) map[string]interface{} {
	for _, f := range _fields {

		if f.Validation != nil && f.Validation.IsPassword { // skip password field
			continue
		}
		...
		if f.FieldType == "multiline" {
			...
			var _nested []*protobuff.FieldInfo
			for _, sf := range f.SubFieldInfo {
				_nested = append(_nested, &protobuff.FieldInfo{
...
func getConnectionsInfo(_connections []*protobuff.ConnectionType) map[string]string {
...
func RESTtoGraphQL(c echo.Context, schema *protobuff.ProjectSchema, rootModel string, rest url.Values, skipQueryDef bool) (*RESTtoGraphResponse, error) {
	...
	case "GET":
		...
		if _id, ok := rest["_id"]; ok {
			...
			queryModelName = inflection.Plural(modelType.Name)
		...
		if relation, ok := rest["relation"]; ok {
			model := inflection.Singular(relation[0])
			delete(rest, "_id") // nested doesnt need id. Must include to avoid loopOnNestedSets
...
func HandleFunction(c echo.Context, schema *protobuff.ProjectSchema, functionName string) (*RESTtoGraphResponse, error) {
	...
	var functionToExecute *protobuff.CloudFunction
