		...
		case "repeated":
			...
			if update {
				fields = append(fields, &models.FieldInfo{
					...
					InputType:   "string",
					FieldType:   "text",
					Validation:  &models.Validation{Required: true, Unique: true},
					//Serial:                  0,
					Label:                   "ID",
...
func OpenApiSpecGenerator(env string, loader *models.ApplicationCache) (interface{}, error) {
	...
	for _, model := range project.Schema.Models {
		...
		pluralName := strings.Title(MultipleResourceName(singularName))
		...
		// list
		{
			...
			openapi.AddOperation(oas.GET, fmt.Sprintf(`/%s`, MultipleResourceName(model.Name)), op)
		}
		...
		// create
		{
			...
			openapi.AddOperation(oas.PUT, fmt.Sprintf("/%s", MultipleResourceName(model.Name)), op)
		}
		...
		// update
		{
			...
			openapi.AddOperation(oas.POST, fmt.Sprintf("/%s", MultipleResourceName(model.Name)), op)
		}
		...
		// delete
		{
			...
			openapi.AddOperation(oas.DELETE, fmt.Sprintf("/%s", MultipleResourceName(model.Name)), op)
		}
		...
		if len(model.Connections) > 0 {
			for _, c := range model.Connections {
				...
				if c.Relation == "has_many" {
					connectedModel = strings.Title(MultipleResourceName(c.Model))
					schemaName = fmt.Sprintf("%s-%s-response", model.Name, strings.ToLower(MultipleResourceName(connectedModel)))
					openapi.AddSchema(schemaName, oas.ObjectOf(oas.Props{
						model.Name: openapi.RefSchema(singularName),
						strings.ToLower(MultipleResourceName(connectedModel)): oas.ItemsOf(openapi.RefSchema(connectedModel)),
					}))
				} else {
	...
	for _, function := range project.Schema.Functions {
		...
		// post
		{
			...
			openapi.AddOperation(oas.POST, fmt.Sprintf("/system/function/%s", MultipleResourceName(function.Name)), op)
		}
	}
