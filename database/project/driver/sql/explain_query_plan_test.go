...
import (
	...
	sysdriver "github.com/apito-io/engine/database/system/driver/sql"
)
...
`).Exec(ctx)
	...
	_ = &SQLDriver{Conf: cfg, ORM: db, DriverCredential: cred}
	...
	// Media list pattern (indexes from execMetaMediaSecondaryDDL)
	qMedia := `SELECT * FROM media AS x WHERE x.model = 'post' ORDER BY x.created_at DESC LIMIT 10 OFFSET 0`
	explainDetailContainsUsingIndex(t, ctx, db, qMedia)
}
