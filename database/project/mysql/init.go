...
// Driver is the MySQL project SQL driver.
type Driver struct {
	sqlcommon.Driver
}

// GetDriver opens a MySQL project driver.
func GetDriver(cfg *models.Config, cred *models.DriverCredentials) (*Driver, error) {
	if cred == nil {
		return nil, sqlcommon.ErrCredentialsRequired()
	}
	ec := *cred
	ec.Engine = _const.MySQLDriver
	base, err := sqlcommon.OpenDriver(cfg, &ec, mysqlDialect{})
	if err != nil {
		return nil, err
	}
