...
func TestCommonDocTransformation_nullMetaColumns(t *testing.T) {
	doc, err := CommonDocTransformation(&models.ModelType{Name: "item"}, "en", map[string]interface{}{
		"id":              "doc-1",
		"sys_status":      nil,
		"sys_created_at":  nil,
		"sys_updated_at":  nil,
	...
	if doc.Meta.Status != "" {
		t.Fatalf("expected empty status, got %q", doc.Meta.Status)
	}
	if doc.Meta.CreatedBy != nil || doc.Meta.LastModifiedBy != nil {
		t.Fatal("expected nil created/updated by when SQL meta columns are null")
	}
}
