...
func (d *Driver) ensureEnableIndexingForModelIDB(ctx context.Context, db bun.IDB, project *models.Project, model *models.ModelType) error {
	if model == nil {
		return nil
	}
	...
	for _, f := range model.Fields {
		...
		if err := d.execCreateIndexDDL(ctx, db, idxParam, id, ""); err != nil {
			return err
		}
	}
	return nil
...
func (d *Driver) ensureEnableIndexingForModel(ctx context.Context, project *models.Project, model *models.ModelType) error {
	if err := d.ensureEnableIndexingForModelIDB(ctx, d.ORM, project, model); err != nil {
		return err
	}
	RunAnalyzeAfterIndexDDL(ctx, d)
	return nil
}
...
func (d *Driver) DropField(ctx context.Context, param *models.CommonSystemParams) error {
	...
	_, err := d.ORM.NewRaw(sql).Exec(ctx)
	if err != nil {
		return err
	}
	return nil
...
func (d *Driver) CreateTableOrCollection(ctx context.Context, param *models.CommonSystemParams, indexes []string) error {
	...
	return d.CreateModelTable(ctx, param.Model, false)
}
...
func (d *Driver) AddModel(ctx context.Context, project *models.Project, model *models.ModelType) (*models.ProjectSchema, error) {
	...
	if d.preferDDLRunInTx() {
		err := d.runDDLBatch(ctx, func(db bun.IDB) error {
			if err := d.createModelTableExec(ctx, db, model, false); err != nil {
				return err
			}
			if err := d.ensureEnableIndexingForModelIDB(ctx, db, project, model); err != nil {
				return err
			}
			return d.installRowCountTriggersForModelTx(ctx, db, model)
		})
		if err != nil {
			return nil, err
		}
	} else {
		if err := d.CreateModelTable(ctx, model, false); err != nil {
			return nil, err
		}
		if err := d.ensureEnableIndexingForModel(ctx, project, model); err != nil {
			return nil, err
		}
		if err := d.installRowCountTriggersForModel(ctx, model); err != nil {
			return nil, err
		}
	}
	if err := RunSQLiteLikePostDDL(ctx, d); err != nil {
		return nil, err
	}
...
func (d *Driver) AddRelationFields(ctx context.Context, from *models.ConnectionType, to *models.ConnectionType) error {
	...
	if d.DriverCredential != nil {
		...
		if err := PreflightSQLiteRelationParentTablesForAddRelation(ctx, d.ORM, eng, from, to); err != nil {
			return err
		}
	}
	...
	runDDL := func(db bun.IDB) error {
		...
		// Bootstrap runs EnsureModelUserFieldColumns before EnsureRelationArtifactsFromSchema;
		// synthetic relation fields may already exist as physical FK columns.
		execAddColumn := func(q string) error {
		...
		case "has_one":
			...
			case "has_one":
				...
				if sqlcommon.EngineIsSQLiteLike(eng) {
					...
					if err := execAddColumn(q2); err != nil {
						return err
					}
	...
	if err := d.runDDLBatch(ctx, runDDL); err != nil {
		return err
	}

	if d.DriverCredential != nil && sqlcommon.EngineIsSQLiteLike(strings.ToLower(strings.TrimSpace(d.DriverCredential.Engine))) {
...
func (d *Driver) DeleteRelationDocuments(ctx context.Context, projectId string, from *models.ConnectionType, to *models.ConnectionType) error {
	...
	eng := strings.ToLower(strings.TrimSpace(d.DriverCredential.Engine))
	sqliteLike := sqlcommon.EngineIsSQLiteLike(eng)
		...
		dropFKColumn := func(table, col, mysqlFKName string, sqliteLogicalModel string) error {
			...
			if sqliteLike {
				...
				if strings.TrimSpace(sqliteLogicalModel) != "" {
					return d.installRowCountTriggersForModelTx(ctx, db, &models.ModelType{Name: sqliteLogicalModel})
				}
				return nil
			}
			if pg {
...
func (d *Driver) AddFieldToModel(ctx context.Context, param *models.CommonSystemParams, isUpdate bool, parent_field string) (*models.ModelType, error) {
	...
	if parent_field == "" && !isUpdate {
		...
		for _, existing := range param.Model.Fields {
	...
	tableName := utility.PhysicalSQLTableName(param.Model.Name)
	stmts, err := AlterTableAddFieldSQL(d.DriverCredential.Engine, tableName, param.FieldInfo)
	if err != nil {
		return nil, err
	}
	err = d.runDDLBatch(ctx, func(db bun.IDB) error {
		for _, query := range stmts {
			if err := d.execRawDDL(ctx, db, query); err != nil {
				return err
			}
		}
		if parent_field == "" && !isUpdate && param.FieldInfo.EnableIndexing {
			...
			return d.execCreateIndexDDL(ctx, db, idxParam, param.FieldInfo.Identifier, "")
		}
		return nil
	})
	if err != nil {
...
func (d *Driver) CreateMediaDocument(ctx context.Context, projectId string, media *models.FileDetails) (*models.FileDetails, error) {
	...
	fileType := models.InferFileTypeFromMIME(media.ContentType)
	...
	if _, err := d.CreateProjectFile(ctx, pf); err != nil {
		return nil, err
	}
	return media, nil
}
...
func (d *Driver) AddDocumentToProject(ctx context.Context, param *models.CommonSystemParams, doc *types.DefaultDocumentStructure) (interface{}, error) {
	...
	err := d.runInTxOrBypass(ctx, func(ctx context.Context, tx bun.IDB) error {
		...
		if err := runDocumentPreInsertHook(d.Conf, ctx, param, data); err != nil {
			return err
		}
		if err := runDocumentPreInsertDocHook(d.Conf, ctx, param, doc); err != nil {
			return err
		}
...
func (d *Driver) UpdateDocumentOfProject(ctx context.Context, param *models.CommonSystemParams, doc *types.DefaultDocumentStructure, replace bool) error {
	...
	return d.runInTxOrBypass(ctx, func(ctx context.Context, tx bun.IDB) error {
		...
		q = applyBunHookWheresUpdate(d.Conf, ctx, param, q)
...
func (d *Driver) DeleteDocumentFromProject(ctx context.Context, param *models.CommonSystemParams) error {
	...
	return d.runInTxOrBypass(ctx, func(ctx context.Context, tx bun.IDB) error {
		...
		q = applyBunHookWheresDelete(d.Conf, ctx, param, q)
		_, err := q.Exec(ctx)
		return err
	})
}
