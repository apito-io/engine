...
func mongoEnsureIndexes(ctx context.Context, coll *mongo.Collection, collName string) error {
	...
	case "project_users":
		if err := mongoCreateIndex(ctx, coll, mongo.IndexModel{
			Keys: bson.D{{Key: "project_id", Value: 1}, {Key: "email", Value: 1}},
		}); err != nil {
			return err
		}
		return mongoCreateIndex(ctx, coll, mongo.IndexModel{
			Keys: bson.D{{Key: "project_id", Value: 1}, {Key: "tenant_id", Value: 1}},
		})
	case "projects":
